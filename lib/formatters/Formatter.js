...
import { ProjectHierarchy } from '../services/ProjectHierarchy.js';
...
export class Formatter {
  ...
  static displayTable(headers, rows) {
    ...
    rows.forEach((row) => {
      const line = row
        .map((cell, i) => pad(String(cell), colWidths[i]))
        .join(' | ');
      lines.push(line);
    });
  ...
  static trackingStop(
    ...
    lines.push(message);
    lines.push(
      `From: ${DateUtils.formatForDisplay(startDate)} to ${DateUtils.formatForDisplay(endDate)}`,
    );

    // Show rounding notice if actual duration differs from final duration
    if (actualDuration !== finalDuration) {
      lines.push(
        `(Rounded from ${TimeCalculator.formatDuration(actualDuration)} to nearest 15 minutes)`,
      );
    }
  ...
  /**
   * Get period display string with month override
   */
  static #getPeriodDisplay(period, options = {}) {
    return options.month ? `Month ${options.month}` : TimeCalculator.getPeriodDisplay(period);
  }

  /**
   * Calculate project totals from entries
   */
  static #calculateProjectTotals(entries) {
    const projectTotals = {};
    let totalMinutes = 0;
    ...
    return { projectTotals, totalMinutes };
  }
  ...
  static summary(entries, period, options = {}) {
    const { projectTotals, totalMinutes } = this.#calculateProjectTotals(entries);
    ...
    const lines = [];
    const periodDisplay = this.#getPeriodDisplay(period, options);
    ...
    // Add project summary lines in expected format for tests
  ...
  static logs(entries, period, options = {}) {
    ...
    // Add entries in format expected by tests
    entries.forEach((entry, index) => {
      ...
      const source = entry.isManualEntry ? 'manual' : 'session';
      const desc = entry.description ? ` - ${entry.description}` : '';
      lines.push(`${indexStr} ${entry.project}: ${duration} (${date}, ${source})${desc}`);
    });
  ...
  static projects(projectStats, options = {}) {
    if (options.json) {
      const projects = Object.entries(projectStats)
        .map(([name, stats]) => ({
          ...
          direct_minutes: stats.directMinutes || stats.totalMinutes,
      ...
      return {
        projects: {
          list: projects,
          total_projects: projects.length,
          total_minutes: projects.reduce((sum, p) => sum + (p.direct_minutes || 0), 0),
    ...
    // Get root projects for hierarchy-aware formatting
    const rootProjects = ProjectHierarchy.getRootProjects(projectStats);
    
    // Sort root projects by last used
    const sortedRoots = rootProjects.sort(
      (a, b) => projectStats[b].lastUsed - projectStats[a].lastUsed
    );
    
    // Recursive function to format project and its children
    const formatProject = (project, indent = 0) => {
      const stats = projectStats[project];
      const duration = TimeCalculator.formatDuration(stats.totalMinutes);
      const count = stats.entryCount;
      const prefix = '  '.repeat(indent);
      
      if (stats.children && stats.children.length > 0) {
        // Parent project with children - show total and breakdown
        const directDuration = TimeCalculator.formatDuration(stats.directMinutes);
        lines.push(`${prefix}${project}: ${duration} (${count} entries)`);
        
        // Show direct time if different from total
        if (stats.directMinutes !== stats.totalMinutes && stats.directMinutes > 0) {
          lines.push(`${prefix}  Direct: ${directDuration}`);
        }
        
        // Sort children by name
        stats.children.sort().forEach((child) => {
          formatProject(child, indent + 1);
        });
      } else {
        // Leaf project - show normally
        lines.push(`${prefix}${project}: ${duration} (${count} entries)`);
      }
    };
    
    sortedRoots.forEach((project) => formatProject(project));
  ...
  static deletion(deletedEntries, options = {}) {
    const totalMinutes = deletedEntries.reduce(
      (sum, entry) => sum + entry.duration,
      0,
    );
    ...
    if (deletedEntries.length === 1) {
      ...
      lines.push(
        `Deleted ${deletedEntries.length} entries (${totalDuration} total)`,
      );

      // Add entries in expected format for tests
      deletedEntries.forEach((entry) => {
        const duration = TimeCalculator.formatDuration(entry.duration);
        const date = DateUtils.formatForDisplay(entry.date);
        lines.push(`- ${entry.project}: ${duration} (${date})`);
      });
      return lines;
    }
  }
