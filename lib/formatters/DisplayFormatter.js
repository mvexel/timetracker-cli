...
export class DisplayFormatter {
  ...
  static formatLogs(entries, period, options = {}) {
    ...
    entries.forEach((entry, index) => {
      const dateStr = DateUtils.formatForDisplay(entry.date);
      const durationStr = TimeCalculator.formatDuration(entry.duration);
      const source = entry.isManualEntry ? '📝 manual' : '⏱️  session';
      
      let message = `[${index + 1}] ${entry.project}: ${durationStr} (${dateStr}) ${source}`;
  ...
  static formatProjects(projectStats, options = {}) {
    ...
    sortedProjects.forEach(([project, stats]) => {
      const lastUsed = DateUtils.formatForDisplay(stats.lastUsed);
  ...
  static formatDeletionMessage(deletedEntries, options = {}) {
    ...
    if (deletedEntries.length === 1) {
      ...
      console.log(`Deleted entry: ${entry.project}: ${duration} (${date})`);
    } else {
      ...
      deletedEntries.forEach(entry => {
        const duration = TimeCalculator.formatDuration(entry.duration);
        const date = DateUtils.formatForDisplay(entry.date);
        console.log(`  - ${entry.project}: ${duration} (${date})`);
      });
    }
  }
   ...
   */
  static formatTrackingStartMessage(project, startTime, options = {}, description = null) {
    if (options.json) {
      const jsonData = JSONFormatter.formatOperation('start', { project, startTime, description });
      JSONFormatter.output(jsonData);
      return;
    }

    let message = `Started tracking project: ${project}`;
    if (description) {
      message += ` - ${description}`;
    }
    message += ` at ${new Date(startTime).toLocaleString()}`;
    console.log(message);
  }
   ...
   */
  static formatTrackingStopMessage(project, actualDuration, finalDuration, startTime, endTime, options = {}, description = null) {
    if (options.json) {
    ...
    let message = `Stopped tracking project: ${project}`;
    if (description) {
      message += ` - ${description}`;
    }
    console.log(message);
    
    if (finalDuration !== actualDuration) {
      console.log(`Duration: ${TimeCalculator.formatDuration(actualDuration)} (rounded to ${TimeCalculator.formatDuration(finalDuration)})`);
    } else {
      console.log(`Duration: ${TimeCalculator.formatDuration(finalDuration)}`);
    }
  }
   ...
   */
  static formatLogEntryMessage(project, duration, date, options = {}, description = null) {
    if (options.json) {
    ...
    console.log(message);
    console.log(`Date: ${DateUtils.formatForDisplay(date)}`);
  }
