...
export class SessionEntry {
  ...
  /**
   * Create SessionEntry from TimeEntry (for migration)
   */
  static fromTimeEntry(timeEntry, sessionId = null) {
    const date = timeEntry.startTime.toISOString().split('T')[0];
    const duration = timeEntry.duration;
    const id = sessionId || SessionEntry.generateSessionId('migr');
    
    return new SessionEntry(
      timeEntry.project,
      date,
      duration,
      null, // no description for migrated entries
      id
    );
  }
}