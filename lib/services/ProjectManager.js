...
import { ProjectHierarchy } from './ProjectHierarchy.js';
...
export class ProjectManager {
  ...
  async listProjects(options = {}) {
    ...
    const hierarchicalStats = ProjectHierarchy.calculateHierarchicalStats(projectStats);
    const result = Formatter.projects(hierarchicalStats, options);
    Formatter.output(result, options.json);
  }
  ...
  async deleteProject(projectName, options = {}) {
    ...
    const matchingEntries = allEntries.filter(
      (entry) => entry.project.toLowerCase() === projectName.toLowerCase(),
    );
    ...
    const result = Formatter.projectDeletion(
      projectName,
      matchingEntries,
      options,
    );
  ...
  async deleteByProject(options) {
    ...
    if (options.project) {
      entriesToDelete = TimeCalculator.filterEntriesByProject(
        entriesToDelete,
        options.project,
      );
      ...
      if (entriesToDelete.length === 0) {
        throw new Error(
          `No entries found for project matching: ${options.project}`,
        );
      }
    }
    ...
    if (options.last) {
      ...
      entriesToDelete = TimeCalculator.filterEntriesByPeriod(
        entriesToDelete,
        period,
        now,
      );
      ...
      if (entriesToDelete.length === 0) {
        const timeRange = options.today
          ? 'today'
          : options.week
            ? 'this week'
            : 'this month';
        const projectFilter = options.project
          ? ` for project matching "${options.project}"`
          : '';
