...
export class ProjectManager {
  ...
  async deleteByProject(options) {
    ...
    if (options.last) {
      ...
      entriesToDelete = TimeCalculator.filterEntriesByPeriod(entriesToDelete, period);
    ...
    const remainingEntries = allEntries.filter(entry => 
      !entriesToDelete.some(deleteEntry => 
        ...
        (deleteEntry.project === entry.project && 
         deleteEntry.startTime.getTime() === entry.startTime.getTime() &&
         deleteEntry.endTime.getTime() === entry.endTime.getTime())
      )
    );
  ...
  #calculateProjectStats(entries) {
    ...
    entries.forEach(entry => {
      ...
      if (entry.startTime > projectStats[project].lastUsed) {
        projectStats[project].lastUsed = entry.startTime;
      }
    });
