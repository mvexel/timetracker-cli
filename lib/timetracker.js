...
class TimeTracker {
  ...
  async summary(period, dateForTesting) {
    ...
    const periodDisplay = this.getPeriodDisplay(period);
    console.log(`
Time Summary - ${periodDisplay}:`);
  ...
  async status() {
    ...
    if (!currentState || !currentState.project) {
      // No output when not tracking - this keeps the prompt clean
      return;
    }
    ...
    // Output format designed for prompt integration
    process.stdout.write(`${currentState.project} (${this.formatDuration(durationMinutes)})`);
  }
}
