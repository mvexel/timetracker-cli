...
export class TimeTracker {
  ...
  async start(project, description = null, options = {}) {
    ...
    DisplayFormatter.formatTrackingStartMessage(project, startTime, options, description);
  }
  ...
  async #autoStop(currentState, options) {
    ...
    const finalDuration = options.noRound ? durationMinutes : TimeCalculator.roundToNearest15(durationMinutes);
    ...
    if (finalDuration > 0) {
      ...
      if (!options.quiet) {
        const message = finalDuration !== durationMinutes ? 
  ...
  async log(projectName, duration, description = null, options = {}) {
    ...
    const targetDate = options.day ? new Date(options.day) : new Date();
    ...
    const entry = new SessionEntry(
      projectName,
      targetDate.toISOString().split('T')[0], // date only
      finalDuration,
      description,
      sessionId
    );
    ...
    await this.#fileManager.appendLogEntry(entry);
    DisplayFormatter.formatLogEntryMessage(projectName, duration, targetDate, options, description);
  }
  ...
  async deleteEntry(index, period = 'all', dateForTesting = null, options = {}) {
    if (isNaN(index)) {
      throw new Error('An entry index to delete is required. Please provide a valid number.');
    }
    const allEntries = await this.#fileManager.readAllLogEntries();
    ...
    const filteredEntries = allEntries.filter(entry => {
      if (entryToDelete.originalLine && entry.originalLine) {
        ...
        return !(entry.project === entryToDelete.project && 
   ...
   */
  async migrate() {
    await this.#fileManager.migrateToNewFormat();
  }
