...
class TimeTracker {
  ...
  async summary(period, dateForTesting) {
    if (!["day", "week", "month", "all"].includes(period)) {
      console.log("Invalid period. Use: day, week, month, or all");
      return;
    }
  ...
  async logs(period, dateForTesting) {
    ...
    if (process.env.NODE_ENV === 'test' || typeof process.stdin.setRawMode !== 'function') {
      this.showStaticLogs(entries, period);
    } else {
      await this.showInteractiveLogs(entries, period);
    }
  }

  showStaticLogs(entries, period) {
    console.log(`\\nInteractive Log Entries - Last ${period}:`);
    console.log("==========================================");
